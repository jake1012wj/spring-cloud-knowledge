package com.jeesite.modules.search.restapi;

import com.jeesite.modules.search.entity.Blog;
import com.jeesite.modules.search.feign.WebFeignClient;
import com.jeesite.modules.search.global.MessageConf;
...
import com.jeesite.modules.search.service.ElasticSearchService;
import com.jeesite.modules.search.utils.ResultUtil;
import com.jeesite.modules.search.utils.StringUtils;
import com.jeesite.modules.search.utils.WebUtils;
...
public class ElasticSearchRestApi {
    ...
    public String initElasticSearchIndex() throws ParseException {
        IndexOperations indexOperations = elasticsearchTemplate.indexOps(ESBlogIndex.class);
        if (indexOperations.exists()) {
            indexOperations.delete(); // 删除索引
        } else {
            System.out.println("索引不存在");
        }
        indexOperations = elasticsearchTemplate.indexOps(ESBlogIndex.class);
        if (!indexOperations.exists()) {
            indexOperations.create(); // 创建索引
            indexOperations.refresh(); // 刷新索引以使更改可见
        } else {
            System.out.println("索引已经存在");
        }
        indexOperations = elasticsearchTemplate.indexOps(ESBlogIndex.class);
        indexOperations.putMapping(ESBlogIndex.class);
        Long page = 1L;
        Long row = 10L;
        Integer size = 0;
